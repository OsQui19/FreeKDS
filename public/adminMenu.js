      ...
      if (currentForm) {
        ...
        if (recipeHidden && recipeHidden.value.trim()) {
          recipeHidden.value.split(/\r?\n/).forEach(step => {
            ...
            const div = document.createElement('div');
            div.className = 'recipe-row d-flex gap-2 mb-1';
            div.innerHTML = `<input class="form-control" type="text" name="recipe_steps" value="${step}">`+
              `<button type="button" class="btn btn-secondary remove-recipe-row">x</button>`;
...
function initDragAndDrop() {
  ...
  new Sortable(document.getElementById('categories-container'), {
    ...
    animation: 150,
    onEnd() {
      const newOrder = Array.from(document.querySelectorAll('#categories-container .category-section'))
  ...
  document.querySelectorAll('.item-list').forEach(listEl => {
    new Sortable(listEl, {
      ...
      onEnd() {
        ...
        const newOrder = Array.from(listEl.querySelectorAll('.menu-item'))
...
function initEditToggles() {
  ...
  document.querySelectorAll('.cancel-cat-edit').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (collapse && collapse.previousElementSibling) collapse.previousElementSibling.classList.remove('hidden');
    });
  });
}
...
function initCollapseText() {
  document.querySelectorAll('.toggle-items-btn').forEach(btn => {
    ...
    if (!target) return;
    const updateText = () => {
      btn.textContent = target.classList.contains('show') ? 'Hide Items' : 'Show Items';
    };
    updateText();
...
function initPersistentModifierSection() {
  ...
  if (urlParams.get('openMods') === '1' || localStorage.getItem('modifierSectionExpanded') === 'true') {
