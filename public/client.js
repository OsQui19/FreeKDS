...
function loadBumpedOrders() {
  return fetch(`/api/bumped_orders?station_id=${stationId}&limit=${MAX_HISTORY}`)
    .then(res => res.json())
    .then(data => {
      ...
      (data.orders || []).forEach(o => {
        ...
        bumpedOrders.push({ orderId: o.order_id, orderNumber: o.order_number, ticket });
      });
    })
    .catch(err => console.error('Failed to fetch bumped orders', err));
}
function createTicketElement({ orderId, orderNumber, orderType, createdTs, specialInstructions, allergy, items }) {
  ...
  const timeStr = d.toLocaleTimeString('en-US', {hour: 'numeric', minute: 'numeric'});
    ...
    fetch(url)
      ...
      .then(data => {
        recipeTextDiv.innerHTML = data.recipe ?
          formatRecipeList(data.recipe) : '<p>No recipe available.</p>';
