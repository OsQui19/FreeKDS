module.exports = function(io, db) {
  ...
  io.on('connection', socket => {
    const registerStation = (stationId) => {
      ...
      db.query('SELECT type FROM stations WHERE id=?', [socket.stationId], (err, rows) => {
        if (!err && rows.length) {
          ...
          console.log('Registered station', socket.stationId, socket.stationType);
        } else {
    ...
    socket.on('bumpOrder', ({ orderId }) => {
      ...
      console.log('bumpOrder', { orderId, stationId: socket.stationId, type: socket.stationType });
      if (socket.stationType === 'expo') {
        db.query('UPDATE orders SET status="completed" WHERE id=?', [orderId], () => {});
        db.query(`INSERT INTO bumped_orders (order_id, station_id, order_number)
        ...
        db.query(`INSERT INTO bumped_orders (order_id, station_id, order_number)
                 ...
                 ON DUPLICATE KEY UPDATE bumped_at=NOW(), order_number=VALUES(order_number)`,
              [socket.stationId, orderId, orderId], () => {});
        io.to('expo').emit('stationDone', { orderId, stationId: socket.stationId });
      }
    });

    socket.on('recallOrder', ({ orderId }) => {
      if (!orderId) return;
      db.query('DELETE FROM bumped_orders WHERE order_id=? AND station_id=?', [orderId, socket.stationId], () => {});
      if (socket.stationType === 'expo') {
        db.query('UPDATE orders SET status="active" WHERE id=?', [orderId], () => {});
        ...
        db.query(fetchSql, [orderId], (err, rows) => {
          if (err) { console.error('Error fetching items for recall:', err); return; }
        ...
        io.to('expo').emit('stationUndo', { orderId, stationId: socket.stationId });
      }
    });
    socket.on('markUrgent', ({ orderId }) => {
      ...
      db.query(sql, [orderId], (err, rows) => {
        if (err) { console.error('Error fetching stations for urgent:', err); return; }
