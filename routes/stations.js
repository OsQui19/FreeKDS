...
const { getBumpedOrders, getStations, getCategories } = require('../controllers/dbHelpers');
module.exports = (db) => {
  ...
  router.get('/station/:id', (req, res) => {
    ...
    db.query('SELECT * FROM stations WHERE id=?', [stationId], (err, stRows) => {
      ...
      if (station.type !== 'expo') {
        ...
        conditions.push('NOT EXISTS (SELECT 1 FROM bumped_orders bo WHERE bo.order_id = o.id AND bo.station_id = ?)');
        orderParams.push(stationId);
      }
      ...
      db.query(orderSql, orderParams, (err3, rows) => {
        ...
        getBumpedOrders(db, stationId, (errB, bumpedOrders) => {
          ...
          getStations(db)
            .then(stationRows => {
            res.render('station', { station, orders, settings, allStations: stationRows, bumpedOrders });
            })
            .catch(err4 => {
        ...
        db.query(sqlItemMods, (err3, itemMods) => {
          ...
          db.query(sqlMods, (err4, mods) => {
            ...
            db.query(sqlGroups, (err5, groups) => {
              if (err5) { console.error(err5); return res.status(500).send('DB Error'); }
              const modMap = {};
              ...
              const catMap = cats.map(c => ({ id: c.id, name: c.name, items: [] }));
              const idx = {};
              catMap.forEach(c => { idx[c.id] = c; });
              items.forEach(it => {
              ...
              res.render('order', { categories: catMap, table, settings: res.locals.settings, modGroups: groups });
            });
          });
        });
        });
